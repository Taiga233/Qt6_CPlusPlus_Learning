/*! @File        : TestQStyledItemDelegate.cpp
 *  @Brief       : copy: TestStandardItemModel
 *  @Details     : Test QStyledItemDelegate
 *  @Author      : Taiga
 *  @Email       : taigaahri@gmail.com
 *  @Date        : 2023-11-23 18:19
 *  @Version     : v1.0
 *
 **********************************************************
 *
...
#include "ui_TestQStyledItemDelegate.h"

#define ACTION_ALIGN_LEFT 1
#define ACTION_ALIGN_CENTER 2
#define ACTION_ALIGN_RIGHT 3

void TestQStyledItemDelegate::initModelData(QStringList &aFileContent)
{
...
void TestQStyledItemDelegate::initAll()
{
    ...
    connect(QISMp_selecetionModel_M, &QItemSelectionModel::currentChanged, this, &TestQStyledItemDelegate::do_currentChanged); //关联选择当前单元格变化槽函数
    ...
    connect(ui->actText_alignLeft, &QAction::triggered, this, qOverload<bool>(&TestQStyledItemDelegate::do_itemsAlignmentTriggered));
...
void TestQStyledItemDelegate::setCustomDelegate()
{
    C_02int_SBDp_m = new SpinBoxDelegate_Custom(this);
    //测深
    ui->TV_01standardItemModel->setItemDelegateForColumn(0, C_02int_SBDp_m);

    C_01float_DSBDp_m = new DoubleSpinBoxDelegate_Custom(this);
    //垂深
    ui->TV_01standardItemModel->setItemDelegateForColumn(1, C_01float_DSBDp_m);
    //方位
    ui->TV_01standardItemModel->setItemDelegateForColumn(2, C_01float_DSBDp_m);
    //总位移
    ui->TV_01standardItemModel->setItemDelegateForColumn(3, C_01float_DSBDp_m);

    C_03combo_CBDp_m = new ComboBoxDelegate_Custom(this);
    QStringList QSL_items_t;
    QSL_items_t << "优" << "良" << "一般" << "不合格";
    C_03combo_CBDp_m->setItems(QSL_items_t, false);
    //固井质量
    ui->TV_01standardItemModel->setItemDelegateForColumn(4, C_03combo_CBDp_m);
}

TestQStyledItemDelegate::TestQStyledItemDelegate(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::TestQStyledItemDelegate)
{
    ui->setupUi(this);
...
TestQStyledItemDelegate::~TestQStyledItemDelegate()
{
    delete ui;
}

void TestQStyledItemDelegate::do_currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    Q_UNUSED(previous);
...
void TestQStyledItemDelegate::do_itemsAlignmentTriggered(bool checked)
{
    Q_UNUSED(checked);
...
void TestQStyledItemDelegate::on_actFile_open_triggered()
...
void TestQStyledItemDelegate::on_actTable_appendRow_triggered()
{
...
void TestQStyledItemDelegate::on_actTable_insertRow_triggered()
{
    QList<QStandardItem *> itemList_t;
    QStandardItem *item_t;
    for (auto i = 0; i < FIXED_COLUMN_COUNT - 1; i++) {
...
void TestQStyledItemDelegate::on_actTable_deleteRow_triggered()
{
...
void TestQStyledItemDelegate::on_actText_fontBold_triggered(bool checked)
{
    if (!QISMp_selecetionModel_M->hasSelection()) return; //没有选择的项则返回
...
void TestQStyledItemDelegate::on_actModel_displayData_triggered()
{
    ui->PTE_01displayData->clear();
    QStandardItem *item_t;
    QString showStr_t;
